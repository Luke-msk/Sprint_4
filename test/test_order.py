...
@allure.description('Используем кнопку "Заказать" в правом верхнем углу и первый набор данных в полях "Станция метро", "Когда привезти самокат", "Срок аренды", "Цвет самоката". В остальных полях данные формируются случайно.')
def test_order_button_above_first(driver):
    ...
    value.order_form_click(order_form_loc.metro_1)
...
@allure.title('Проверка оформления заказа')
@allure.description('Используем кнопку "Заказать" в правом верхнем углу и второй набор данных в полях "Станция метро", "Когда привезти самокат", "Срок аренды", "Цвет самоката". В остальных полях данные формируются случайно.')
def test_order_button_above_second(driver):
    value = Order_form_page(driver)
    main = MainPage(driver)
    confirm = ConfirmationOrder(driver)
    main.click_on_button(main_page_loc.cookie)
    ...
    value.order_form_send(order_form_loc.field_address, config.address)
    value.order_form_click(order_form_loc.field_metro)
    value.order_form_click(order_form_loc.metro_2)
    value.order_form_send(order_form_loc.field_phone_number, config.phone_number)
    value.order_form_click(order_form_loc.button_further)
    value.find_elements(order_form_loc.form_about_rent)
    assert value.find_text(order_form_loc.form_about_rent) == order_form_loc.form_about_rent_text
    value.order_form_click(order_form_loc.date)
    value.order_form_click(order_form_loc.date_fifth_day)
    value.order_form_click(order_form_loc.rental_period)
...
@allure.title('Проверка оформления заказа')
@allure.description('Используем кнопку "Заказать" внизу страницы и первый набор данных в полях "Станция метро", "Когда привезти самокат", "Срок аренды", "Цвет самоката". В остальных полях данные формируются случайно.')
def test_order_button_below_first(driver):
    value = Order_form_page(driver)
    main = MainPage(driver)
    confirm = ConfirmationOrder(driver)
    main.click_on_button(main_page_loc.cookie)
...
@allure.title('Проверка оформления заказа')
@allure.description('Используем кнопку "Заказать" внизу страницы и второй набор данных в полях "Станция метро", "Когда привезти самокат", "Срок аренды", "Цвет самоката". В остальных полях данные формируются случайно.')
def test_order_button_below_second(driver):
    value = Order_form_page(driver)
    main = MainPage(driver)
    confirm = ConfirmationOrder(driver)
    main.click_on_button(main_page_loc.cookie)
